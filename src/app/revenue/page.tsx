...
import {
  sampleMonthlyRevenue, sampleWeeklyRevenue, sampleRevenueByTerritory,
  sampleTopProducts, sampleRevenueByCategory, sampleTopCustomers,
} from '@/lib/sample-data';
import { formatCurrency, formatNumber } from '@/lib/utils';

export default function RevenuePage() {
  const currentMonth = sampleMonthlyRevenue[sampleMonthlyRevenue.length - 1];
  const prevMonth = sampleMonthlyRevenue[sampleMonthlyRevenue.length - 2];
  const growth = ((currentMonth.revenue - prevMonth.revenue) / prevMonth.revenue) * 100;
  const totalYtd = sampleMonthlyRevenue.reduce((s, r) => s + r.revenue, 0);

  const revenueByCustomer = sampleTopCustomers.map((c) => ({
    ...
    <div className="flex flex-col">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
          {[
            { label: 'This Month', value: formatCurrency(currentMonth.revenue), sub: `${growth >= 0 ? '+' : ''}${growth.toFixed(1)}% vs last month`, subColor: growth >= 0 ? 'text-green-600' : 'text-red-600' },
            { label: 'Last Month', value: formatCurrency(prevMonth.revenue), sub: `${prevMonth.orders} orders`, subColor: 'text-gray-500' },
            { label: 'YTD Revenue', value: formatCurrency(totalYtd), sub: '12-month total', subColor: 'text-gray-500' },
            { label: 'Avg Monthly', value: formatCurrency(totalYtd / 12), sub: 'rolling average', subColor: 'text-gray-500' },
          ].map((stat) => (
          ...
          <CardContent>
            <Tabs defaultValue="monthly">
              ...
              <TabsContent value="weekly">
                <RevenueLineChart data={sampleWeeklyRevenue} height={280} />
              </TabsContent>
              <TabsContent value="monthly">
                <RevenueLineChart data={sampleMonthlyRevenue} height={280} />
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Current month by sales region</CardDescription>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="amount">
                ...
                <TabsContent value="amount">
                  <TerritoryBarChart data={sampleRevenueByTerritory} height={240} showGrowth={false} />
                </TabsContent>
                <TabsContent value="growth">
                  <TerritoryBarChart data={sampleRevenueByTerritory} height={240} showGrowth={true} />
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <CategoryPieChart data={sampleRevenueByCategory} height={280} />
              <div className="mt-4 space-y-2">
                {sampleRevenueByCategory.map((cat) => (
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Top 5 customers this month</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Top 8 products this month</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3 mt-2">
                {sampleTopProducts.map((p, i) => {
                  const maxRevenue = sampleTopProducts[0].revenue;
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Last 12 months detailed breakdown</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {[...sampleMonthlyRevenue].reverse().map((row, i, arr) => {
                    const prev = arr[i + 1];
